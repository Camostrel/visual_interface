...
## ⛔ config.js через scp НЕ ДЕПЛОИТЬ (правило с 2026-08-21)

`js/config.js` на диске держит **заглушку токена** (`PASTE_LONG_LIVED_ACCESS_TOKEN_HERE`),
а на стенде в нём лежит **рабочий long-lived token**. `scp` этого файла затирает токен,
и интерфейс на стенде молча теряет связь с HA — выглядит как «сломался деплой».

Локальный `config.js` отличается от стендового **ровно одной строкой** — номером версии.
Поэтому деплой версии — это точечная правка на месте, а не заливка файла:

```bash
ssh ... root@office.arvid-cloud.ru \
  "sed -i 's/APP_VERSION = \"v0.12.1\"/APP_VERSION = \"v0.13.0\"/' \
   /config/www/NickSha/visual_interface/js/config.js"
```

Проверить, что токен на месте (значение не печатаем):
```bash
ssh ... "grep -c PASTE_LONG_LIVED /config/www/NickSha/visual_interface/js/config.js"   # ждём 0
```

> Если токен всё же затёрт — восстановить его может только владелец: значение есть в
> `/config/www/NickSha/web_interface/js/config.js` (тот же HA) либо выпускается заново
> в профиле HA. Заранее снимать копию файла перед правкой — дешевле, чем искать потом.
