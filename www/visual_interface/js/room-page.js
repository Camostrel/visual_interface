...
class ArvidRoomPage {
  ...
  async loadRoomSvg() {
    ...
    // не знает своего размера — вписывать было бы не во что.
   ...
   *
   * Здесь остаётся одно: снова вписать план целиком, когда область изменила
   * размер (поворот экрана, появление/исчезновение карточки, разворот опций). Если пользователь
