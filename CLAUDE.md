...
функциональность вычищена под узкий скоуп. Текущая версия фронта: **v0.8.1**, backend **v0.1.0**.
...
### Здоровье устройств — истина ядра DALI (v0.8.0)

Своей логики offline/аварий **не пишем**. `js/health.js` дёргает `arvid_dali_center/health_data`
(read-only, без `require_admin`) и раскладывает записи по помещениям.

- `kind` → `offline` (пульс) / `anomaly` (янтарь) / `gateway` (строка в «Предупреждениях»).
- **Поллинг** 300 с, в «Диагностике» 30 с, пауза при скрытой вкладке. Push-канала нет.
  Чаще не нужно: ядро не флагает ошибку первые `interval_min` (5 мин) — грейс против транзиентов.
- Ядра нет (`unknown_command`) → модуль отключается, остальные режимы работают.
- ⚠ `health_data` отдаёт **имя** помещения, не `area_id` → резолв через реестр; дубли имён
  не сопоставимы. Запрос к ядру — **H6** в [DEBT.md](docs/DEBT.md).
