...
функциональность вычищена под узкий скоуп. Текущая версия фронта: **v0.7.0**, backend **v0.1.0**.
...
**Где мы: v0.7.0** (backend v0.1.0). Свой backend (домен `visual_interface`, чистый стор).
...
**Следующее:** Фаза 2 — логика режимов карты (Присутствие / Диагностика); селектор уже есть.
