...
**Где мы: v0.10.2** (backend v0.1.0). Скоуп свет/датчики/панели закрыт.
...
**Закрыто:** Фаза 1, Фаза 2, Фаза 3.5. Долги D2, D3, D4, D6, D9, D13, D18, H6.

**⚠ Аудит v0.10.2 нашёл 4 проблемы P0 — они НЕ починены (см. [DEBT.md](docs/DEBT.md) §P0):**
- **A1** — конвертер плана может привязать имя лампы к СОСЕДНЕМУ светильнику (у 8 подписей
  отрыв от второго кандидата < 5 ед.), а его контроль качества не срабатывает никогда;
- **A2** — не схлопнутая геометрия (значок без подписи, чужой слой) уезжает за пределы плана;
- **A3** — WebSocket не переподключается: после обрыва интерфейс показывает застывшую
  картинку и молча не реагирует на тапы (лечится только F5);
- **A4** — `layout/save` затирает весь документ без блокировки и версии: смена темы на одном
  устройстве стирает расстановку, сохранённую на другом.

Правки P0 **компоновать по месту** (один патч на файл/подсистему), а не по пунктам.
...
**Следующее:** P0 из аудита (A1/A2 — конвертер, A3 — реконнект WS, A4 — layout);
Фаза 3 (остаток) — зоны по фактической геометрии вместо прямоугольников;
индексы реестра (D20) и только ПОТОМ замер памяти на 166 устройствах + порог зума
(сейчас 2.2 «на глаз»); D1/D19 — масштаб (`subscribe_entities` вместо «всё сразу»).
