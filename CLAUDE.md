...
**Где мы: v0.10.1** (backend v0.1.0). Скоуп свет/датчики/панели закрыт.
...
**Закрыто:** Фаза 1, Фаза 2, Фаза 3.5. Долги D2, D3, D4, D6, D13, D18, H6.
...
**⚠ Ждём от пользователя (иначе план серый):** в HA создать area с id `kab_301/302/303`,
переименовать 8 ламп и датчики под имена из плана, создать группы `light.kab_30x`.
Если фактические `area_id` выйдут другими — перегенерировать зоны (одна команда).

**Следующее:** Фаза 3 (остаток) — зоны по фактической геометрии вместо прямоугольников;
замер памяти/быстродействия на 166 устройствах + подбор порога зума (сейчас 2.2 «на глаз»);
D1/D19 — масштаб (`subscribe_entities` вместо «всё сразу», индекс area в реестре).
